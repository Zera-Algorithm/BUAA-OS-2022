...
/* Overview:
   ...
   then create it.*/
// Actually only use high 22bit of VA.
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	// PTE_ADDR: removes the low 12 bit of the addr.
	pgtable = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp)); // pgtable is kernel virtual address.
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	for (i = 0, size = ROUND(size, BY2PG); i < size; i += BY2PG) {
		pgtable_entry = boot_pgdir_walk(
			pgdir,
			va + i, /* current virtual page */
			1 /* create if not exists. */
		);
		*pgtable_entry = ((pa + i)&~0xFFF) | perm | PTE_V;
	}
...
void page_init(void)
{
	 ...
	 * filed to 1) */
	for (i = 0; i < PADDR(freemem); i += BY2PG) {
		pa2page(i)->pp_ref = 1;
	}
...
int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	/* Step 1: Get the corresponding page directory entry and page table. */
	pgdir_entryp = pgdir + PDX(va);
	pgtable = (Pte *)PTE_ADDR(*pgdir_entryp);
